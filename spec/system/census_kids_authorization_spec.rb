...
describe "Authorizations with census kids", :perform_enqueued, with_authorization_workflows: ["census_kids_authorization_handler"] do
  let(:organization) do
    create(
      :organization,
  ...
  before do
    ...
    allow_any_instance_of(CensusKidsAuthorizationHandler).to receive(:response).and_return(response)
  ...
  context "when user account" do
    ...
    it "allows the user to authorize against available authorizations" do
      ...
      click_on "El padró (majors de 8 anys)"
      ...
      within ".authorizations-list" do
        ...
        expect(page).to have_no_link("El padró (majors de 8 anys)")
      end
    end
    ...
    context "when the user has already been authorised" do
      let!(:authorization) do
        create(:authorization,
               name: CensusKidsAuthorizationHandler.handler_name,
               user:)
      end
