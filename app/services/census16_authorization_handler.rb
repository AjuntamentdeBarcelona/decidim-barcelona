...
class Census16AuthorizationHandler < Decidim::AuthorizationHandler
  ...
  def census_document_types
    %i(dni nie passport).map do |type|
      [I18n.t(type, scope: "decidim.census16_authorization_handler.document_types"), type]
    end
  end
  ...
  def document_type_valid
    ...
    errors.add(:document_number, I18n.t("census16_authorization_handler.invalid_document")) unless response.xpath("//codiRetorn").text == "01"
  end
