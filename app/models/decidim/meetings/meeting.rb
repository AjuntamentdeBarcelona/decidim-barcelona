  ...
  module Meetings
    ...
    class Meeting < Meetings::ApplicationRecord
      ...
      def visible_for_admin_user?(user)
        ...
        table_name = self.component.participatory_space.class.table_name
        roles_table = "#{table_name.singularize}_user_roles"
        return unless ActiveRecord::Base.connection.table_exists?(roles_table)

        query = "SELECT decidim_user_id FROM #{roles_table} WHERE decidim_user_id = #{user.id} AND #{table_name.singularize}_id = #{self.component.participatory_space.id} AND role = 'admin'"
