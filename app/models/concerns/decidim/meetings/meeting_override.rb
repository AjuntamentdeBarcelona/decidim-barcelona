    ...
    module MeetingOverride
      ...
      included do
        def visible_for_admin_user?(user)
          ...
          query = "SELECT decidim_user_id FROM #{roles_table} WHERE decidim_user_id = #{user.id} AND #{table_name.singularize}_id = #{component.participatory_space.id} AND role = 'admin'"
