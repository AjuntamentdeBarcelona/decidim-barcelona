    ...
    dataReady = function() {//This is called as soon as data is loaded
		...
		a.iterNodes(
			function (b) { //This is where we populate the array used for the group select box
				...
				// alert(b.x);
				//console.log("aaaaaaaaaaaaa" + JSON.stringify(b.attr.attributes.District) );
...
function setupGUI(config) {
	...
	if (config.logo.file) {
		...
		logo = "<img width=\"225\" height=\"auto\" src=\"" + config.logo.file +"\"";
...
function Search(a) {
    ...
    this.search = function (a) {
        ...
        this.lastSearch = a;
        this.results.empty();
        if (1 >= a.length) this.results.html("<i>Ha de buscar un nom amb un mínim d'una lletra.</i>");
        else {
...
function nodeActive(a) {
	...
	if (groupByDirection) {
		...
		f.push("<h2>Mutus (" + size + ")</h2>");
		(size>0)? f=f.concat(createList(mutual)) : f.push("No hi ha enllaços mutus<br>");
		size=Object.size(incoming);
		f.push("<h2>Entrants (" + size + ")</h2>");
		(size>0)? f=f.concat(createList(incoming)) : f.push("No hi ha enllaços entrants<br>");
		size=Object.size(outgoing);
		f.push("<h2>Sortints (" + size + ")</h2>");
		(size>0)? f=f.concat(createList(outgoing)) : f.push("No hi ha enllaços sortints<br>");
	} else {
    ...
    if (f.attributes) {
		...
		/*console.log(d);
		console.log(d.startsWith('http'));
		if (d.startsWith('http')>0) {
			d = '<a target="_blank" href="' + d + '">' + d + '</a>';
		}*/
